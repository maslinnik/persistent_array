...
struct PersistentArrayBase {
  ...
  class BaseIterator {
    ...
    friend class PersistentArrayBase;
  ...
  explicit PersistentArrayBase(std::shared_ptr<BaseNode> root)
      : root(std::move(root)) {}
  ...
  static PersistentArrayBase filled(const T& fill) {
    return PersistentArrayBase{build_filled(0, N, fill)};
  }

  template <std::input_iterator Iter>
  static PersistentArrayBase from_iter(Iter first) {
    return PersistentArrayBase{build_from_iter(0, N, first)};
  }

  template <typename... Args>
  PersistentArrayBase update(size_t index, Args&&... args) const {
    auto new_root =
        updated_node(root.get(), index, std::forward<Args>(args)...);
    return PersistentArrayBase{std::move(new_root)};
  }
};
